...
class ReviewAgentDefinition extends AgentDefinition {
  ...
  buildSystemPrompt(agent: AgentConfig, config: AutonomyConfig): string {
    ...
    return [
      ...
      '',
      'Treat that file as the project-specific context pack. It exists so you do not need to rediscover the whole repo before every gate pass.',
      '',
      '## Role',
      '',
  ...
  private async executeRunnerReview(context: AgentExecutionContext, work: ClaimedReviewWork): Promise<ExecutionResult> {
    ...
    const scopeResult = context.scopeEvaluator.evaluate
      ...
      : { ok: true, violations: [] };
    const reviewCheckCommands = resolveReviewCheckCommands(
      reviewContext.worktreePath,
      pr.checks || [],
      reviewDiffFiles,
      pr.acceptance || reviewerTask.acceptance || [],
    );
