import type { PrdSourceChat } from '../../types.js';

const SOURCE_CHAT_HEADER_RE = /^#{2,6}\s+Source Chat Message\s*$/im;
const NEXT_MARKDOWN_HEADER_RE = /(?:^|\r?\n)#{1,6}\s+\S/;

function extractPrdSourceChatMetadata(prd: unknown): PrdSourceChat | null {
  if (!isRecord(prd)) {
    return null;
  }
...
function extractPrdSourceChatMetadataFromSpecification(specification: unknown): PrdSourceChat | null {
...
function normalizePrdSourceChat(value: unknown): PrdSourceChat | null {
  if (!isRecord(value)) {
    return null;
  }
...
function parseSourceChatSection(section: string): PrdSourceChat | null {
  const source: PrdSourceChat = {};
...
function mergePrdSourceChat(
  base: PrdSourceChat | null,
  override: PrdSourceChat | null
) {
  if (!base && !override) {
    return null;
  }
...
function compactPrdSourceChat(source: PrdSourceChat): PrdSourceChat | null {
