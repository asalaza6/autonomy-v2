...
function defaultGithubApiRunner(
  ...
  const script = [
    ...
    'const accept = String(payload.accept || "application/vnd.github+json").trim() || "application/vnd.github+json";',
    'const defaultHeaders = {',
    '  Authorization: `Bearer ${token}`,',
    '  Accept: accept,',
    '  "Content-Type": "application/json",',
    '  "User-Agent": "autonomy-v2-control-plane-github",',
    '  "X-GitHub-Api-Version": "2022-11-28",',
    '};',
    'function fail(status, statusText, body) {',
    '  const detail = body ? `\\n${body}` : "";',
    '  console.error(`HTTP ${status} ${statusText}${detail}`.trim());',
    '  process.exit(1);',
    '}',
    'function parseGraphqlArgs(values) {',
    '  let query = "";',
    '  const variables = {};',
    '  for (let index = 1; index < values.length; index += 1) {',
    '    const flag = String(values[index] || "");',
    '    const rawValue = String(values[index + 1] || "");',
    '    if (!flag.startsWith("-")) {',
    '      continue;',
    '    }',
    '    if (flag === "-f" || flag === "--raw-field") {',
    '      const separator = rawValue.indexOf("=");',
    '      const key = separator >= 0 ? rawValue.slice(0, separator) : rawValue;',
    '      const value = separator >= 0 ? rawValue.slice(separator + 1) : "";',
    '      if (key === "query") {',
    '        query = value;',
    '      } else if (key) {',
    '        variables[key] = value;',
    ...
    '      variables[key] = Number.isFinite(numeric) && String(numeric) === value ? numeric : value;',
    '      index += 1;',
    '    }',
    '  }',
    ...
    'const request = target === "graphql"',
    '  ? (() => {',
    '      const parsed = parseGraphqlArgs(args);',
    '      return {',
    '        url: "https://api.github.com/graphql",',
    '        init: { method: "POST", headers: defaultHeaders, body: JSON.stringify(parsed) },',
    '      };',
    '    })()',
    '  : {',
    '      url: `https://api.github.com/${target.replace(/^\\/+/, "")}`,',
    '      init: { method: "GET", headers: defaultHeaders },',
    '    };',
  ...
  return execFileSync(process.execPath, ['--input-type=module', '-e', script, JSON.stringify(payload)], {
    encoding: 'utf8',
